...
For directional solidification, an example including automated input file generation, launching of an ensemble of ExaCA simulations and analysis of the simulation results, the plotting of surrogate model response surfaces for two outputs of interest from the grain analysis (mean grain area and mean grain extent in the Z direction), and the calibration of the model input parameters (thermal gradient and nucleation density) can be run using the python script [utilities/TASMANIAN/DirSolCalibration.py](utilities/TASMANIAN/DirSolCalibration.py)
